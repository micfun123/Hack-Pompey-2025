...
# Variable to select PM type
PM_TYPE = "PM1"  # Options: "PM1", "PM10", "PM25"
daynumber = 0  # Day number to select from the JSON data
...
c = gaussian_plume(x_grid, y_grid, first_entry[PM_TYPE], first_entry["Wind Speed"], first_entry["Wind Dir"],
                    sigma_y, sigma_z)

# Make black areas fully transparent
plt.figure(figsize=(6, 6), dpi=300, facecolor='none')
plt.imshow(c, cmap='hot', interpolation='nearest', alpha=0.50, origin='lower', extent=[-500, 500, -500, 500])
plt.axis('off')
plt.savefig("gaussian_plume.png", bbox_inches='tight', pad_inches=0, transparent=True)
plt.close()
